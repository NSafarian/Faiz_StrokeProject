...
Uninj_data_dir <- "/Path/to/Data/Directory/"
Stroke_data_dir <- "Path/to/Data/Directory/"
...
## **Visualize the expression levels of selected cluster-7 markers in 10X single-cell driven clusters and on the visium dataset** 
...
### *Step3) Plot the expression levels*
```{r, warning = FALSE}

# 1) Visualize the expression levels of select markers among all single-cell driven clusters
FeaturePlot(seu.combined,
            reduction = "umap",
            features = c("Apoe", "Cd81", "Fabp5", "Aldoc", "Clu"),
            order = TRUE,
            pt.size= 0.5, 
            min.cutoff = 'q10',
            cols = c("#E7ECF0","#F5C245","#EB6359","#696267","#529085"), # you may add other DEG to this list
            label = TRUE,
            label.size=5,
            repel=T, ncol = 3) &
  theme(axis.title.x= element_text(face="bold", size=10),
...
# 2) Compare the expression levels of select markers between the two clusters of interest
## 2.1) Subset Seurat object 
seu.cluster6and7 <- subset(seu.combined, subset=new_clusters %in%c("6","7"))

## 2.2) Heatmap expression levels of the single-cell data
...
## 2.3) Spatial mapping on Visium data (day10)
